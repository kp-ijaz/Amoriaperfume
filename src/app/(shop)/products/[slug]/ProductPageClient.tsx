    ...
    <div className="flex gap-2.5">
      ...
      <div className="flex flex-col gap-2 flex-shrink-0" style={{ width: 60 }}>
        {images.map((img, i) => (
          ...
          >
            <Image src={img.url} alt={`${name} ${i + 1}`} fill className="object-contain p-1" />
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 relative">
        <div className="relative w-full overflow-hidden" style={{ aspectRatio: '1/1', backgroundColor: '#F8F8F8' }}>
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2">
            <button
              type="button"
              className="min-h-12 min-w-12 bg-white rounded-full flex items-center justify-center shadow-sm hover:shadow-md transition-shadow"
              aria-label="Share product"
              onClick={() => navigator.share?.({ title: name, url: window.location.href })}
            >
              <Share2 size={14} style={{ color: '#555' }} />
            </button>
            <button type="button" className="min-h-12 min-w-12 bg-white rounded-full flex items-center justify-center shadow-sm hover:shadow-md transition-shadow" aria-label="Add to wishlist">
              <Heart size={14} style={{ color: '#555' }} />
            </button>
          </div>

          {/* Bottom-right zoom */}
          <button type="button" className="absolute bottom-3 right-3 min-h-12 min-w-12 bg-white rounded-full flex items-center justify-center shadow-sm" aria-label="Zoom image">
...
export function ProductPageClient({
  ...
  const { data: fetchedProduct, isLoading, isError } = useApiProductBySlug(slug);
      ...
      <div className="rounded p-3 mb-4 flex flex-col gap-2" style={{ backgroundColor: '#FAFAF8', border: '1px solid #EEEBE6' }}>
        ...
        <div className="flex items-center gap-2 text-[12px]" style={{ color: '#44403C' }}>
          ...
          <a
            ...
            className="text-[11px] underline mr-1"
            style={{ color: '#78716C' }}
            aria-label="Learn more about Tabby pay in 4 payments"
          >
