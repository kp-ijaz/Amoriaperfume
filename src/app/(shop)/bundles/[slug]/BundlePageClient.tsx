...
import { ChevronRight, Layers, Package, ShoppingBag, Zap, CheckCircle } from 'lucide-react';
import { toast } from 'sonner';
import { useBundleBySlug } from '@/lib/hooks/useBundles';
import { useCart } from '@/lib/hooks/useCart';
import { bundleToPackageCartItem } from '@/lib/cart/packageCartItem';
...
export function BundlePageClient({ slug }: { slug: string }) {
  ...
  const hasDiscount = activeBundle.mrp > activeBundle.bundlePrice;
  const savings = hasDiscount ? activeBundle.mrp - activeBundle.bundlePrice : 0;
  const itemDetails = (activeBundle.items ?? []).map(resolveBundleItemDetails);
  const inStock = isBundleInStock(activeBundle.items ?? []);
  const canAddToCart = inStock && itemDetails.length > 0;
  const bundleName = activeBundle.name;

  const detailsText =
    activeBundle.description?.trim() ||
    `This curated bundle includes ${itemDetails.length} ${
      itemDetails.length === 1 ? 'fragrance' : 'fragrances'
    } at a special combined price. Every item is authentic and ready to ship.`;

  async function addBundleToCart() {
    if (!canAddToCart) return;
    setAddedState('loading');
    await new Promise((r) => setTimeout(r, 350));
    addPackageItem(bundleToPackageCartItem(activeBundle));
    ...
    <div style={{ backgroundColor: '#FAF8F5', minHeight: '100vh' }}>
      <div className="max-w-6xl mx-auto px-4 py-6">
        <nav className="flex items-center gap-1.5 text-xs mb-8" style={{ color: '#6B6B6B' }}>
          ...
          <ChevronRight size={12} />
          <span style={{ color: '#1A0A2E' }}>{activeBundle.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
          ...
          >
            <PackageGallery images={activeBundle.images ?? []} name={activeBundle.name} />
          </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-end gap-3 flex-wrap">
                <p className="text-3xl font-semibold" style={{ color: '#1A0A2E' }}>
                  {formatCurrency(activeBundle.bundlePrice)}
                </p>
                {hasDiscount && (
                  <>
                    <p className="text-sm line-through pb-1" style={{ color: '#9B9B9B' }}>
                      {formatCurrency(activeBundle.mrp)}
                    </p>
            ...
            >
              {[
                { icon: '🚚', label: 'Free Shipping', sub: 'Orders over AED 200' },
            ...
            <Link
              ...
              style={{ color: '#1A0A2E' }}
            >
              Back to Bundles <ChevronRight size={13} />
            </Link>
          </motion.div>
        </div>
